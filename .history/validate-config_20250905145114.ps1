...
function Parse-AzureSectionFromEnvFile {
    ...
    $result = [ordered]@{ AppName = $null; ResourceGroup = $null }
    ...
    try {
        ...
        foreach ($raw in $lines) {
            ...
            if ($inSection) {
                ...
                if ($line -match '^# ==+' ) { break }
                if ($line -match '^# *- *App Name:\s*(.+)$') { $result.AppName = $Matches[1].Trim() }
                elseif ($line -match '^# *- *Resource Group:\s*(.+)$') { $result.ResourceGroup = $Matches[1].Trim() }
                # Defensive stop if both gathered
                if ($result.AppName -and $result.ResourceGroup) { break }
            }
        }
    } catch { }
    return $result
...
# Extract Azure AppName / ResourceGroup from commented section
$azureNames = Parse-AzureSectionFromEnvFile -FilePath '.\\.env'
if ($azureNames.AppName -or $azureNames.ResourceGroup) {
    Write-Status "🔧 Parsed Azure config from .env comments: AppName='${($azureNames.AppName | ForEach-Object { $_ })}' ResourceGroup='${($azureNames.ResourceGroup | ForEach-Object { $_ })}'" "Info"
}
